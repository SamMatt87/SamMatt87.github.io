...
## Issues covered
### Bed not sticking
In this issue, the initial layer of filament has not stuck to the bed of the printer. This is usually shown by stringy filament being dragged arount the bed or bunching up around the nozzle, potentially causing a clog. The issue can be fixed in two ways, either adjusting the heat of the bed or applying glue to the bed before th print. There are 94 images in this set.
...
### Defining the autoencoder
Before I built the autoencoder, I needed to define factors like the number of epochs, the learning rate and the batch size. I also had to initialise the autoencoder with the input size, nodes sizes window and step size for tiling and the latent dimension size (As these variables are usualy trial and error, I wanted to make the autoencoder builder as modular as possible). We can then compile and run the autoencoder as shown in the code below.
...
### Saving outputs
Finally, there are a number of output of this file that need to be saved including:
...
## Autoencoder
An autoencoder is a neural network that attempts to recreate the image it was intially given. This is acheived through passing the image through an encoder, then passing the output back through a decoder to produce a new image. Due to the informed guess and check nature of building a convolutional neural network model, I wanted to make sure the autoencoder was as modular as possible. As such, the function accepts parameters for the image height, width and depth(number of channels), the number of nodes in each convolutional layer, the window and stride fro tiling, and the latent dimension for the dense layer of the encoder.

### Encoder
The encoder starts with an input layer based on the shape of the image. The code then cycles through each node creating a convolutional layer for each based on the tiling parameters with relu activation. After the final node, the network is flattened to be fed into a dense layer based on the latent dimensions input. Thus final output along with the input is then saved as the encoder model. You can see the code for this below.
...
### Calculate errors
The next step is to calculate the overall and by pixel errors in each image. The overall mean squared error allows us to identify the worst reconstructed images. The error by pixel allows us to create a heatmap showing the worst performing areas of each image. After running the images through the models prediction function, we calcualt the overal mean squared error, stored as a float and the mean squared error for each pixel, stored as a numpy array. You can see the code for this section below.
...
### Share results
Each image amongst this new list is checked against its label to see if it was one of the defect images. The results show that with the threshold set to 98%, the model finds 6 of the 7 defect images. For a more qualitative assessment, the code also writes the original, reproduction, and heatmap for each image in the error range to a new file. after some preprocessing, the heatmap is generated using the `applyColormap` function of the cv2 package. The original, reconstruction and heatmap are then stacked horizontally for each image and each image is stacked vertically. This ouput is then written to a file using cv2. This is done using the code below.
